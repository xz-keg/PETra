...
def load_checkpoint(model, optimizer, opt_param_scheduler, load_arg='load', strict=True):
    ...
    load_kwargs["checkpoint_step"]=args.ckpt_step
    ...
    #state_dict['model']['decoder.final_layernorm._extra_state']=None
    if len(model) == 1:
        #print(list(state_dict['model'].keys()))
        #print(state_dict['model']['decoder.layers.0.self_attention.core_attention._extra_state'])
        #print(model[0])
        filtered_state_dict = {k: v for k, v in state_dict['model'].items() if not k.endswith('_extra_state')}
        model[0].load_state_dict(filtered_state_dict, strict=False)
    else:
        for i in range(len(model)):
            mpu.set_virtual_pipeline_model_parallel_rank(i)
