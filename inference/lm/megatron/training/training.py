...
def read_model(model_provider_func,
                              model_type,
                              no_wd_decay_cond=None,
                              scale_lr_cond=None,
                              lr_mult=1.0):
    ...
    model = get_model(model_provider_func, model_type)
    unwrapped_model = unwrap_model(model)
    state_dict=load_file(args.load+'/model.safetensors')
    print(model)
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                for loss_dict in loss_dicts:
                    for key in loss_dict:
                        ...
                        val = loss_dict[key]
                        #print("added to total loss dict:",key,val)
                        if isinstance(val, tuple) or isinstance(val, list):
...
def pretrain_mutgpt(train_iterator,valid_iterator,
             model_provider,
             model_type,
             forward_step_func,
             process_non_loss_data_func=None,
...
def evaluate_mutgpt(data_iterator,
             model_provider,
             model_type,
             forward_step_func,
             eval_func,
    ...
    total_loss_dict = eval_func(
        forward_step_func, data_iterator, model,config)
    # Timelimit hit during evaluation
    print(total_loss_dict)
    if timelimit:
        return
    prefix=args.ckpt_step
