...
def get_ltor_masks_and_position_ids(data,
    ...
    # attention mask for mutation embedding (each mutation is encoded to 9 tokens, these 9 tokens form 1-2-3-3 groups that do not link with each other to avoid leakage) 
    for q in range((seq_length-5)/9):
        attention_mask[:,:,9*q+6:9*q+14,9*q+5]=0
        attention_mask[:,:,9*q+8:9*q+14,9*q+6:9*q+8]=0
        attention_mask[:,:,9*q+11:9*q+14,9*q+8:9*q+11]=0
    print("set up specific att mask")
    # Loss mask.
